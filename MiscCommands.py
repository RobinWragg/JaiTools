...
class JaiToolsReadMeCommand(sublime_plugin.WindowCommand):
  def run(self):
...
class JaiToolsShowBuildResultsCommand(sublime_plugin.WindowCommand):
  def run(self):
    if PANEL_KEY_build_results in self.window.panels():
      self.window.run_command('show_panel', {'panel': PANEL_KEY_build_results})
    else:
      sublime.error_message('JaiTools: No build results exist for this session yet.')
